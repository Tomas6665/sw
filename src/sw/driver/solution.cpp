...
Solution::Solution(const SwContext &swctx)
    : swctx(swctx), base_ptr(*this)
{
...
bool Solution::isKnownTarget(const LocalPackage &p) const
{
...
path Solution::getSourceDir(const LocalPackage &p) const
{
    return p.getDirSrc2();
}
    ...
    for (auto &[p, t] : getChildren())
    {
        ...
        for (auto &d : nt->Dependencies)
        {
            ...
            if (d->target && !d->IncludeDirectoriesOnly)
                s += "\"" + p.toString() + "\"->\"" + d->target->getPackage().toString() + "\";\n";
        }
    }
    s += "}";
    write_file(p, s);
